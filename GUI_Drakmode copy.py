...
class VideoConverterGUI:
    def __init__(self, root):
        ...
        self.style = tb.Style()
        monitor_system_theme(self.style, self.root)
    ...
    def convert_all_videos(self):
        ...
        if not self.cancelled:
            if self.is_dangerous_path(input_dir):
                messagebox.showwarning("安全警告", f"此路径过于敏感，已禁止自动删除：\n{input_dir}")
                self.status_label.config(text=self.status_label.cget("text") + "\n⚠️ 已跳过敏感路径删除")
            else:
                confirm = messagebox.askyesno(
                    "删除源文件夹",
                    f"转换完成，共处理 {self.total_files} 个视频文件。\n\n是否删除以下源文件夹及其所有内容？\n\n路径：\n{input_dir}"
                )
                if confirm:
                    try:
    ...
    def is_dangerous_path(self, path):
        path = os.path.abspath(path).lower()
        home = os.path.expanduser("~").lower()
        risky_dirs = ['desktop', 'downloads', 'documents', 'pictures', 'music', 'videos']
        if path in ['c:\\', 'd:\\']:
            return True
        for risky in risky_dirs:
            if os.path.join(home, risky) in path:
                return True
        return False

if __name__ == '__main__':
    root = tb.Window(title="视频批量转换器", themename="flatly")
